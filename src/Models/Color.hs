...
getAllColors = do
  ...
  where
    queryString = "SELECT id, label FROM colors;"

getColor :: ColorId -> IO (Maybe Color)
getColor paramId = do
  withConn $ \conn -> query conn queryString [paramId] >>= resultsToMaybeColor
  where
    queryString = "SELECT id, label FROM colors WHERE id = ? LIMIT 1"
...
createColor paramLabel = do
  ...
  where
    queryString = "INSERT INTO colors (label) VALUES (?) RETURNING id, label"
...
updateColor paramId paramLabel = do
  withConn $ \conn -> do
    let updateList = catMaybes [const "label = ?" <$> paramLabel]
        ...
        selectQueryString = "SELECT id, label FROM colors WHERE id = ? LIMIT 1"
...
deleteColor paramId = do
  withConn $ \conn -> query conn queryString [paramId] >>= resultsToMaybeColor
  where
    queryString = "DELETE FROM colors WHERE id = ? RETURNING id, label"
