...
getAllDrawers userId paramStorageUnitId = do
  ...
  where
    queryString =
      [sql| SELECT id, user_id, storage_unit_id, level, note
            FROM drawers
            WHERE user_id = ?
              AND storage_unit_id = ? |]
  ...
  where
    queryString =
      [sql| INSERT INTO drawers (user_id, storage_unit_id, level, note)
            VALUES (?, (SELECT id FROM storage_units where id = ? AND user_id = ?), ?, ?)
            RETURNING id, user_id, storage_unit_id, level, note |]
...
updateDrawer userId paramId paramStorageUnitId paramLevel paramNote = do
  withConn $ \conn -> do
    let updateList =
          catMaybes
            [ const [sql| storage_unit_id = ? |] <$> paramStorageUnitId,
              const [sql| level = ? |] <$> paramLevel,
              const [sql| note = ? |] <$> paramNote
            ]
        paramList =
          catMaybes
        ...
        selectQueryString = [sql| SELECT id, user_id, storage_unit_id, level, note FROM drawers WHERE id = ? AND user_id = ? LIMIT 1 |]
