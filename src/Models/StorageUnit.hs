...
getAllStorageUnits userId paramRoomId = do
  ...
  where queryString = "SELECT id, user_id, room_id, name FROM storage_units WHERE user_id = ? AND room_id = ?;"
...
getStorageUnit userId paramId = do
  ...
  where queryString = "SELECT id, user_id, room_id, name FROM storage_units WHERE id = ? AND user_id = ? LIMIT 1"
...
createStorageUnit userId paramRoomId paramName = do
  ...
  where queryString = "INSERT INTO storage_units (user_id, room_id, name) VALUES (?, (SELECT id FROM rooms where id = ? AND user_id = ?), ?) RETURNING id, user_id, room_id, name"
...
deleteStorageUnit userId paramId = do
  withConn $ \conn -> query conn queryString (paramId, userId) >>= resultsToMaybeStorage
  where queryString = "DELETE FROM storage_units WHERE id = ? AND user_id = ? RETURNING id, user_id, room_id, name"

-- helper functions

resultsToMaybeStorage :: [(Integer, UserId, Integer, String)] -> IO (Maybe StorageUnit)
resultsToMaybeStorage = \case
  [(resId, resUserId, resRoomId, resName)] -> return $ Just $ StorageUnit { sId = resId
                                                                          , sUserId = resUserId
                                                                          , sRoomId = resRoomId
                                                                          , sName = resName }
  _ -> return Nothing
